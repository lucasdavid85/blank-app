...
function element(id){if(!elements.has(id))elements.set(id,{tagName:'DIV',value:id==='edit-layer'?'track':'',checked:true,width:700,height:700,style:{},classList:{toggle:noop},addEventListener:function(k,fn){this[k]=fn;},getContext:()=>context,getBoundingClientRect:()=>({left:0,top:0,width:700,height:700}),setPointerCapture:noop});return elements.get(id);}
const ctx=vm.createContext({THREE,document:{getElementById:element,createElement:()=>element('temp')},addEventListener:noop,innerWidth:1200,innerHeight:900,performance:{now:()=>0},console,URL,Blob,setTimeout,Float32Array,requestAnimationFrame:noop});
...
resetKart();mapOpen=false;keys.add('arrowup');for(let i=0;i<600;i++)step(1/120);keys.clear();if(!gate.broken)throw Error('Gate cannot be broken by driving from spawn');
...
console.log('Passed: gate collision, driving from start, reset, labels and annotations');
